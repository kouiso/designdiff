...
- **Non-Existent File Paths**: Always verify file paths with `find`/`ls` before citing. Never guess.
- **Numbered Review Comments**: ❌ `## [MUST] #1: Title` (GitHub auto-links #1). ✅ `## [MUST]: Title`.
- **Missing Line Numbers**: Always include verified line numbers.
- **Code Review Without Purpose**: Follow: (1) What the code does → (2) Why it's a problem → (3) Alternative.
- **Skipping Quality Checks Because CI Passed**: CI pass is minimum bar, not quality assurance.
- **Reviewing Stale Diffs**: Verify current file state with `git show origin/<branch>:<path>` before commenting.
- **Review Without "Better Way" Perspective**: Review for: (1) Correctness → (2) Quality → (3) Optimization → (4) Maintainability.
...
4. **GitHubには書き込まない** → ユーザーの明示的な許可なしに投稿禁止
