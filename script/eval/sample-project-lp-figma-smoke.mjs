...
const figmaManifest = resolve(
  optionalString(
    options,
    "figma-manifest",
    join(repoDir, "verification/fixture/sample-project-lp-figma-pages.template.json"),
  ),
);
...
if (realRun) {
  const readinessArgs = [
    join(repoDir, "script/eval/sample-project-lp-figma-readiness.mjs"),
    "--lp-repo",
    lpRepo,
    "--figma-manifest",
    figmaManifest,
...
try {
  ...
  if (realRun) {
    ...
    await run("node", [join(repoDir, "script/eval/figdiff-cluster-bench.mjs")], {
      env: {
        ...process.env,
