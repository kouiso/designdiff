...
const checkRounds = (ledger) => {
  ...
  if (!first || !second) errors.push("Both execution rounds are required");
    ...
    // 両巡は同一ホストで並列に回りうるので「先に終わる」ことは要求しない。
    // 2巡目が1巡目より後に開始宣言されていることだけは同一実施の見分けに必要。
    if (Date.parse(second.startedAt) <= Date.parse(first.startedAt))
      errors.push("Round 2 must start after round 1");
  }
