...
`verification/fixtures/` は FigDiff v2 の golden fixture 置き場です。L7 の accuracy correlation 層で、人手判断と `DiffReport.aggregateVerdict` の相関を継続監視する前提で管理します。
...
4. 画像がコード生成可能なら `verification/scripts/generate-fixtures.mjs` に生成ロジックを追加する
5. `pnpm --filter @figdiff/mcp-server test` で fixture runner を通す

## 再生成

```bash
node verification/scripts/generate-fixtures.mjs
```
