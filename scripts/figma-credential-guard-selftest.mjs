...
const guardScript = resolve("scripts/figma-credential-guard.mjs");
...
try {
  ...
  await writeFixture(
    ...
    '{ "scripts": { "test": "node scripts/token-contract-smoke.mjs && node scripts/background-error-contract-smoke.mjs" } }\n',
  );
  await writeFixture(
    cleanRoot,
  ...
  await writeFixture(
    badRoot,
    "app/chrome-extension/package.json",
    '{ "scripts": { "test": "node scripts/token-contract-smoke.mjs" } }\n',
  );
  await writeFixture(
    badRoot,
    "app/chrome-extension/src/popup.ts",
