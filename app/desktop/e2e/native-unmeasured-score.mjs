...
const evidencePath = process.env.FIGDIFF_SCORE_EVIDENCE;
if (!evidencePath) throw new Error("FIGDIFF_SCORE_EVIDENCE is required");
...
await writeFile(
  join(convergenceDirectory, "fixture.json"),
  JSON.stringify(
    ConvergenceHistorySchema.parse({
      ...
      campaigns: [
        {
          ...
          iterations: [
            {
              comparisonId: "fixture-iteration",
              matchRate: 50,
              regionCount: 1,
              status: "UNCERTAIN",
...
const environment = {
  ...
  FIGDIFF_CONVERGENCE_DIR: convergenceDirectory,
...
const sanitizeConsoleMessage = (message) =>
  message.replace(/'data:font\/[^']+'/g, "'[embedded font data]'");
try {
  const page = await application.firstWindow();
  page.on("pageerror", (error) => pageErrors.push(error.message));
  page.on("console", (message) => {
    if (message.type() === "error") consoleErrors.push(sanitizeConsoleMessage(message.text()));
  });
  ...
  const checkScore = async (name, count) => {
    ...
    for (let index = 0; index < count; index++) {
      ...
      const color = await values
        .nth(index)
        .evaluate((element) => element.style.color.replaceAll(" ", ""));
      assert.equal(color, before ? "var(--diff)" : "var(--muted-fg)");
    }
    if (!before)
      await expect(page.getByRole("img", { name: "未実行", exact: true })).toHaveCount(count);
    await page.screenshot({
      path: join(evidence, `${before ? "before" : "after"}-${name}.png`),
      animations: "disabled",
    });
  };
  ...
  await page.screenshot({
    path: join(evidence, `${before ? "before" : "after"}-compare.png`),
    animations: "disabled",
  });
  if (!before) {
    await page.getByRole("button", { name: "ノード", exact: true }).click();
    await expect(
      page.getByText("Figmaデザインを読み込むとノードを検査できます。", { exact: true }),
    ).toBeVisible();
    await page.screenshot({
      path: join(evidence, "after-node-inspection-empty.png"),
      animations: "disabled",
    });
    await page.getByRole("button", { name: "課題", exact: true }).click();
  }
  await page
  ...
  await page.screenshot({
    path: join(evidence, `${before ? "before" : "after"}-measured.png`),
    animations: "disabled",
  });
  if (!before) {
    await page
      .getByRole("navigation", { name: "Main navigation" })
      .getByText("収束", { exact: true })
      .click();
    await expect(page.getByText("design.png · review-alpha", { exact: true })).toHaveCount(2);
    await page.screenshot({
      path: join(evidence, "after-convergence.png"),
      animations: "disabled",
    });
  }
  ...
  await writeFile(
    join(evidence, `${before ? "before" : "after"}-native.json`),
    JSON.stringify(
      {
        native: true,
        preload: true,
