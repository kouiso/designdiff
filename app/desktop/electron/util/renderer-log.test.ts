...
describe("redactSecrets", () => {
  ...
  it("閉じ引用符の無い値が次の行を巻き込まないこと", () => {
    ...
    // 値に空白があっても後半が残らないこと (引用符なしの分岐に落ちると最初の空白で止まる)。
    const kv = 'password="my super secret\n[main] user said "hello" to nobody\n[main] next line';
    expect(redactSecrets(kv)).toBe(
      'password=***\n[main] user said "hello" to nobody\n[main] next line',
    );
    const single = "password='my super secret\n[main] user said 'hi' here";
    expect(redactSecrets(single)).toBe("password=***\n[main] user said 'hi' here");
    const json = '{"password":"my super secret\n[main] said "hello": 1\n[main] next';
