...
- **X05/X07 Android実証**（stdio-android-verification.mjs 新規・d0d3de38/2218a15c）: macminiに Pixel 7 Pro (USB) + x08emu が接続済みと判明し blocked 見込を撤回。
  - macOS (`android-mac-r6`): 2台同時接続で serial 省略を明示拒否、emulator 1080×2400 / Pixel 1080×2340 を serial 指定で個別撮影（adb直接撮影と寸法一致）、不存在 serial 明示拒否、emulator scroll 10枚→1080×12350 結合・reachedBottom。実機scrollも `android-mac-r5` で12枚→10644pxを別途確認。
  - WSL (`android-wsl-r6`) / Windows (`android-win-r1`): `adb tcpip 5555` + mac側承認済み adbkey 複製で Pixel を `<android-lan-ip>:5555` として直接接続（リモート adb server は多端末で不安定のため不採用）。1台環境では serial 省略の自動選択・実機撮影・不存在 serial 拒否・実機 scroll 12枚→11359/11352px 結合を確認。
...
- **desktop D-case実E2E**（app/desktop/e2e/desktop-d-cases.mjs 新規, 2338cfdf）: D01/D03/D08/D09/D10 + X09-desktop を実Electron UIで1本のdriverとして実行。合成figma境界 (FD01: canvas 7:7 配下に frame 7:8/7:9 + エラーモード注入用 7:10-7:15) で実fetchを記録し、FigDiff自身のmatchRateには依存しない。
  ...
  - **Round-6 (issue #169) で driver 全面修正**: 旧 driver の誤判定10件 (D04の3モード→全7モード実ピクセル照合、transform変更だけで合格していたzoom/pan/resetの数値照合、初回crop検証不足、waitForFunction引数誤り、早期pageerror欠落、tokenless境界がcredential保存前で終了、開始時completed:false未永続化、session計測の実効なし、証跡の改訂紐付け欠落) を修正。証跡は `evidence.json` の `meta` (runId/driverSha256/repoSha/repoDirty/platform) と run 紐付け、D04含む必須7keysを構造assert。linux 実走証跡 `docs/evidence/desktop-d-linux-r1` (completed:true, 7keys, selftest早期エラー検出・probe非default帰属確認済)。**旧 driver での3platform PASSは旧oracleのものであり、新driverでのWSL/macOS/Windows再実行は未実施 — 未PASS扱い**（結果の「6/6」とD04内の7項目は別レイヤの数値）。compare UIは FLOW_VIEW_MODES を7モードへ拡張 (driverがUI未到達の4モードを検証するため)。なお blended_diff の期待画素は現行 drawBlendedDiff の挙動 (difference合成drawでもglobalAlphaをリセットしない) に固定しており、製品が修正された場合は driver 期待値の更新が必要 — 同描画クセは製品側の別改善候補。
