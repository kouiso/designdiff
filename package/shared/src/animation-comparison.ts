...
const validateMatchRate = (matchRate: number) => {
  if (!Number.isFinite(matchRate) || matchRate < 0 || matchRate > 1) {
    throw new Error(
      "compareOne returned an invalid matchRate; expected a finite value from 0 to 1.",
    );
  }
  return matchRate;
};
...
async function compareEachFrame(
  ...
  for (const frame of implFrames) {
    ...
    const matchRate = validateMatchRate(outcome.matchRate);
    frames.push({
      atMs: frame.atMs,
...
const buildStaticComparisonResult = (
  frames: FrameComparison[],
  frameTimeSource: AnimationCompareInput["frameTimeSource"],
...
export async function runAnimationCompare(
  ...
  if (designFrames.length <= 1) {
    const frames = await compareEachFrame(
      designFrames[0]?.path ?? input.designSource,
      input.implFrames,
      compareOne,
    );
    return buildStaticComparisonResult(frames, input.frameTimeSource);
  }
  ...
  const comparedByTime = new Map<number, FrameComparison>();
  const matchedStatuses: FrameComparison["status"][] = [];
  ...
  for (const designFrame of designFrames) {
    ...
    const comparisonsForDesign = new Map<number, FrameComparison>();
    for (const candidate of candidates) {
      ...
      measured.push({ atMs: implFrame.atMs, mismatchRate: 1 - matchRate });
      comparisonsForDesign.set(implFrame.atMs, comparison);
      // 同じ実装時刻が複数の設計時刻の候補になることがある。合否は、
      // いちばんよく合った設計時刻のものを残す。パスは時刻ごとに再利用できるため識別子にしない。
      const previous = comparedByTime.get(implFrame.atMs);
      if (previous === undefined || comparison.matchRate > previous.matchRate) {
        comparedByTime.set(implFrame.atMs, comparison);
      }
    }
    const alignment = alignFrame(designFrame.atMs, measured);
    alignments.push(alignment);
    // 別の設計への最良結果で、この設計への不一致を上書きしない。
    const selected =
      alignment.matchedAtMs === null ? undefined : comparisonsForDesign.get(alignment.matchedAtMs);
    if (selected !== undefined) matchedStatuses.push(selected.status);
  }

  for (const comparison of comparedByTime.values()) frames.push(comparison);
