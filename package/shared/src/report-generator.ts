...
import type { CompareDesignResult } from "./type.js";
...
export function generateMarkdownReport(result: CompareDesignResult): string {
  ...
  if (result.clusterCollapse) {
    ...
    // 領域の抽出件数だけでは、画素差分や判定保留がないとは言えない。
    lines.push(
      "No localized diff regions were reported. See the pixel counts and review status above.",
    );
  } else {
